...
@export var KEY_BIND_SPRINT := "sprint"
...
func move_player(delta):
	...
	if Input.is_action_pressed(KEY_BIND_SPRINT) and is_on_floor():
	...
	velocity.x = move_toward(velocity.x, direction.x * resultSpeed, accel * delta)
	velocity.z = move_toward(velocity.z, direction.z * resultSpeed, accel * delta)
