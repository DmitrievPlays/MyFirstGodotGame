...
public partial class PlayerController : CharacterBody3D
{
    ...
    [Export]
    private GridContainer Hotbar; // player hotbar

    private int lastSelectedIndex;

    private Resource inHand; // player is now holding in hand

    [Export]
    private Node3D _playerHand; // player hand
    ...
    public override async void _Ready()
    {
        ...
        Inventory.AddItem(await resourceManager.GetResource(OriginalResources.ResourceType.GOLD), 12);
        Inventory.AddItem(await resourceManager.GetResource(ModResources.ResourceType.AXE), 2);
        Inventory.AddItem(await resourceManager.GetResource(OriginalResources.ResourceType.STREETLIGHT), 2);
        Inventory.AddItem(await resourceManager.GetResource(OriginalResources.ResourceType.COAL), 20);
        Inventory.AddItem(await resourceManager.GetResource(OriginalResources.ResourceType.WIRE), 32);

        Inventory.OnInventoryChanged += RenderHand;
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        if (Input.IsActionJustPressed("inventory"))
        {
            ...
            inventoryHolder.ShowScreen([screen], this.HEAD);
        }
        ...
        if (@event is InputEventKey key)
        {
            if (key.Pressed)
            {
                try
                {
                    ...
                    if (index >= 0 && index < 8)
                    {
                        Hotbar.GetChild(index).Set(Button.PropertyName.Disabled, true);
                        Hotbar.GetChild(lastSelectedIndex).Set(Button.PropertyName.Disabled, false);

                        inHand = Inventory.GetItems()[index + 1].Resource;
                        lastSelectedIndex = index;

                        RenderHand(null, null);
                    }
                }
        ...
        if (Input.IsActionJustPressed("throw"))
        {
            ...
            if (inHand is IPlacable placable)
            {
                ObjectPlacer.Activate(inHand);
            }
        }
    ...
    private void RenderHand(object sender, EventArgs e)
    {
        if (inHand is not null)
            _playerHandTexture.Texture = ResourceLoader.Load<Texture2D>(@$"{inHand.Icon}");
        else
            _playerHandTexture.Texture = null;
    }
}
